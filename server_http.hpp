...
namespace SimpleWeb {
  ...
  public:
	...
	/// If not any match within the upper map,this method is called
	std::function<void(std::shared_ptr<typename ServerBase<socket_type>::Response>, std::shared_ptr<typename ServerBase<socket_type>::Request>)> default_callback;
  ...
  protected:
    ...
    void find_resource(const std::shared_ptr<Session> &session) {
	  ...
	  if (it != default_resource.end())
		  ...
		  write(session, default_callback);
    }
